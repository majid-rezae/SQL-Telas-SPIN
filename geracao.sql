...
SELECT  [DATE] ,round(([Potencia]*24)/1000,2) as [MWh]  from (SELECT TOP (7) T.[DATE] 
      ,round(T.[PREDICTION],2) as [PREDICTION_TEMPERATURA],
	  round(P.[PREDICTION],2) as [PREDICTION_PRESSAO],
	  round(V.[PREDICTION],2) as [PREDICTION_VELOCIDADE],
	  ((P.[PREDICTION] *100)/ ((T.[PREDICTION]+273.15)*287.058)) as [Densidade],
	  T.[SystemNumber]  as [PREDICSystemNumber] ,
CASE WHEN V.[PREDICTION] <= 3.05 THEN 
0	   
WHEN V.[PREDICTION] >= 14.5 THEN 
1850
...
0.066047* power(V.[PREDICTION] ,6)   - 5.247376* power(V.[PREDICTION] ,5)   + 172.339049* power(V.[PREDICTION] ,4)   - 2988.955128* power(V.[PREDICTION] ,3)   + 28791.781162* power(V.[PREDICTION] ,2)  - 145473.363921* power(V.[PREDICTION] ,1)   + 301262.823738
  ...
  join  [ActionETL].[dbo].[DT_FORECAST_VELOCIDADE] as [V]
    on P.[DATE] = V.[DATE] 
	 	  where T.[SystemNumber]=@AEG
	  and V.[SystemNumber]=@AEG
	  and P.[SystemNumber]=@AEG
...
BEGIN
 ...
 begin
 SELECT  [DATE] ,round( sum(( ([Potencia])*24)/1000),2) as [MWh]  from (SELECT TOP (7*@count) T.[DATE] 
      ,round(T.[PREDICTION],2) as [PREDICTION_TEMPERATURA],
	  round(P.[PREDICTION],2) as [PREDICTION_PRESSAO],
	  round(V.[PREDICTION],2) as [PREDICTION_VELOCIDADE],
	  ((P.[PREDICTION] *100)/ ((T.[PREDICTION]+273.15)*287.058)) as [Densidade],
...
SET @count2 =  (select count(distinct(SystemNumber)) from  [ActionETL].[dbo].[DT_FORECAST_TEMPERATURA]  WHERE   [SystemNumber] in (SELECT [SystemNumber] FROM [ActionNet_BW].[dbo].[Base_Comum] WHERE ([Usina]='BW1' or [Parque]='BW1' or [Usina]='BW2' or [Parque]='BW2') and Equipamento like 'AEG%')  )
...
BEGIN
	...
	IF @CGE   IN ('TODOS')
 begin
 SELECT  [DATE] ,round(((sum([Potencia])*24)/1000),2) as [MWh]  from (SELECT TOP (7*@count2) T.[DATE] 
      ,round(T.[PREDICTION],2) as [PREDICTION_TEMPERATURA],
	  round(P.[PREDICTION],2) as [PREDICTION_PRESSAO],
	  round(V.[PREDICTION],2) as [PREDICTION_VELOCIDADE],
	  ((P.[PREDICTION] *100)/ ((T.[PREDICTION]+273.15)*287.058)) as [Densidade],
...
0.066047* power(V.[PREDICTION] ,6)   - 5.247376* power(V.[PREDICTION] ,5)   + 172.339049* power(V.[PREDICTION] ,4)   - 2988.955128* power(V.[PREDICTION] ,3)   + 28791.781162* power(V.[PREDICTION] ,2)  - 145473.363921* power(V.[PREDICTION] ,1)   + 301262.823738
  ...
  join  [ActionETL].[dbo].[DT_FORECAST_VELOCIDADE] as [V]
    on t.[DATE] = V.[DATE] and t.[SystemNumber]=v.SystemNumber
	 WHERE  t.[SystemNumber] in (SELECT [SystemNumber] FROM [ActionNet_BW].[dbo].[Base_Comum] WHERE ([Usina]='BW1' or [Parque]='BW1' or [Usina]='BW2' or [Parque]='BW2') and Equipamento like 'AEG%'  ) 
